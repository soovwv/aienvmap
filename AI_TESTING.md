...
2. Ask the human for approval before running the exact trial command supplied by the maintainer. On Windows PowerShell, explain and use `npx.cmd aienvmap@0.2.1 trial` when `npx` would resolve to a policy-blocked `npx.ps1`; keep every package argument unchanged.
3. Run `npx aienvmap@0.2.1 trial`, or the Windows PowerShell `npx.cmd` equivalent above, only in the human-approved disposable directory or disposable project copy. Read and obey that exact version's `schema --json` `outputs.trial` contract. Version 0.1.1 is a legacy trial with broader project-local writes and must not be substituted.
