...
test("external case intake requires portable evidence, human verification, and privacy review", async () => {
  ...
  for (const required of ["--portable --json", "--portable-from", "Human verification", "False positive", "not a repository fixture", "did not paste raw reconcile", "linkable pseudonymous", "authorize maintainers"]) assert.match(template, new RegExp(required, "i"));
