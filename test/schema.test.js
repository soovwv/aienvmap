...
test("schemaContract describes stable AI output contracts", () => {
  ...
  assert.ok(schema.outputs.reconcile.nvmNodeInventoryFields.includes("installationCount"));
  assert.deepEqual(schema.outputs.reconcile.nvmNodeRelationships, ["configured-root-version-path-match", "inventory-version-match", "managed-root-inference", "unconfirmed"]);
  ...
  assert.match(schema.outputs.reconcileHomes.command, /--inspect-homes/);
  assert.match(schema.outputs.reconcileHomes.extractCommand, /--home-evidence/);
  assert.match(schema.outputs.reconcileHomes.mode, /up to eight explicit homes/);
  assert.match(schema.outputs.reconcileHomes.privacyRule, /system users are never enumerated/);
  ...
  assert.deepEqual(schema.recommendation.firstCommands, ["aienvmap start", "aienvmap onboard", "aienvmap context --json"]);
  ...
  assert.equal(schema.readOrder[0], ".aienvmap/discovery.json");
  assert.equal(schema.readOrder[1], ".aienvmap/README.md");
  assert.equal(schema.readOrder[2], ".aienvmap/status.json");
  ...
  assert.equal(schema.agentDiscovery.optionalInstallCommand, "aienvmap onboard --agents cursor,copilot");
  assert.match(schema.agentDiscovery.fallbackPrompt, /Use aienvmap as the workspace env map/);
  ...
  assert.deepEqual(schema.agentDiscovery.fallbackRead, [".aienvmap/discovery.json", ".aienvmap/README.md", ".aienvmap/status.json", ".aienvmap/summary.md", "aienvmap context --json"]);
  ...
  assert.equal(schema.sbomStrategy.defaultCommand, "aienvmap sbom --json");
  assert.equal(schema.sbomStrategy.scannerCommand, "aienvmap sync --security");
  ...
  assert.match(schema.releaseReadiness.currentBatch.changes.join(" "), /generated \.aienvmap\/README\.md and summary\.md surface aiEntry/);
  ...
  assert.equal(schema.dashboard.discoveryFallback.command, "aienvmap start --json");
  assert.equal(schema.dashboard.discoveryFallback.entry, ".aienvmap/discovery.json");
  assert.ok(schema.dashboard.discoveryFallback.decisionValues.includes("fallback-required"));
  assert.equal(schema.dashboard.discoveryFallback.nextSetupCommand, "aienvmap onboard");
  ...
  assert.equal(schema.outputs.status.contract.name, "aienvmap-preflight");
  assert.equal(schema.outputs.discover.command, "aienvmap discover --json");
  ...
  assert.equal(schema.outputs.start.command, "aienvmap start --json");
  ...
  assert.equal(schema.outputs.startHere.file, ".aienvmap/README.md");
  assert.match(schema.outputs.startHere.purpose, /discover the \.aienvmap directory/);
  ...
  assert.equal(schema.outputs.summary.command, "aienvmap summary --write");
  ...
  assert.equal(schema.outputs.plan.command, "aienvmap plan --json");
  ...
  assert.equal(schema.outputs.sbom.command, "aienvmap sbom --json");
  ...
  assert.ok(schema.outputs.scorecard.rootFields.includes("marketReadiness"));
  assert.ok(schema.outputs.scorecard.rootFields.includes("releaseAssessment"));
  ...
  assert.match(schema.compatibility.readOrderRule, /\.aienvmap\/discovery\.json/);
  ...
  assert.match(schema.compatibility.sessionStartRule, /\.aienvmap\/discovery\.json/);
...
test("schemaWorkspace prints JSON without requiring a workspace", async () => {
  ...
  assert.equal(schema.outputs.summary.file, ".aienvmap/summary.md");
  assert.equal(schema.outputs.discovery.file, ".aienvmap/discovery.json");
  assert.equal(schema.outputs.sbom.file, ".aienvmap/sbom.json");
  assert.equal(schema.outputs.cyclonedxLite.file, ".aienvmap/sbom.cdx.json");
  assert.equal(schema.agentDiscovery.startCommand, "aienvmap start");
  assert.equal(schema.agentDiscovery.installCommand, "aienvmap onboard");
  assert.equal(schema.agentDiscovery.discoverCommand, "aienvmap discover");
  assert.equal(schema.agentDiscovery.fallbackCommand, "aienvmap start --json");
