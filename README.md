...
- Runtime routing evidence: `npm.runtimeLinks` and `python.runtimeLinks` connect commands to likely runtimes, while full scans use bounded Volta plain inventory plus Node's reported executable for exact image ownership. Routing links keep `ownershipProven: false`; even proven Volta ownership never grants removal permission.
...
- Python manager proof: the same explicit deep scan uses offline uv managed-Python inventory plus bounded `pyenv root`/local-version evidence. Exact uv interpreter or pyenv prefix matches set `ownershipProven: true`, while `removalAuthorized` always remains false; default scans keep known manager paths as inference only.
