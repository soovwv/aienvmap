...
- Existing environment: run `npx aienvmap reconcile`; it reports visible Node/npm/pnpm/Yarn/Corepack and Python/pip installations, package locations, inventory digests/samples, project expectations, and lockfile conflicts without changing anything. Compare redacted cases offline with `--portable-compare before.json --against after.json --json`; differences never authorize changes.
