...
- Existing environment: run `npx aienvmap reconcile`; it reports visible Node/npm/pnpm/Yarn/Corepack and Python/pip/uv/pipx/Conda installations, package locations, inventory digests/samples, project expectations, and routing conflicts without changing anything. Compare redacted cases offline with `--portable-compare before.json --against after.json --json`; differences never authorize changes.
