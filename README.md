...
- Existing environment: run `npx aienvmap reconcile`; it reports visible Node/npm and Python/pip installations, package locations, inventory digests/samples, project expectations, and lockfile conflicts without changing anything. Use `--portable --json` for quick redacted evidence, or preserve reviewed full evidence with `--portable-from .aienvmap/reconcile.json --json`.
