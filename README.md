...
- Existing environment: run `npx aienvmap reconcile`; it reports visible Node/npm and Python/pip installations, package locations, inventory digests/samples, project expectations, and lockfile conflicts without changing anything. Portable outputs include a canonical `aerp1` evidence fingerprint for comparison—not a machine ID—and remain review-before-sharing.
