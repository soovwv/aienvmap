...
- Runtime routing evidence: `npm.runtimeLinks` and `python.runtimeLinks` connect commands to likely runtimes, while full scans use bounded Volta/fnm/mise inventories plus reported Node paths for exact ownership. Routing links never prove ownership; manager proof never grants removal permission.
