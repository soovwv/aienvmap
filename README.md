...
- Existing environment: run `npx aienvmap reconcile`; it reports visible Node/npm/pnpm/Yarn/Corepack and Python/pip/uv/pipx/Conda installations, package locations, inventory digests/samples, project expectations, and routing conflicts without changing anything. On a shared server, `--inspect-home /absolute/user/home` scans bounded known roots for that readable home plus host roots while excluding the invoking PATH, redacting home paths, and disabling deep/raw-path modes. Compare redacted cases offline with `--portable-compare before.json --against after.json --json`; differences never authorize changes.
