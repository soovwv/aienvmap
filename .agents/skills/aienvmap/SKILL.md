...
If the user confirms intentional versions and wants the answer remembered for this workspace, record only the explicit reviewed version set in `.aienvmap/policy.yml`, for example `intentional-node-versions: 20,22`, `intentional-python-versions: 3.11,3.12`, or `intentional-java-versions: 17,21`. Never write this policy before the user answers. A newly detected version or any routing/project mismatch still requires review. Java remains information-only and is never added to consolidation candidates.
