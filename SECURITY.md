...
The manual npm workflow requests GitHub OIDC permission and publishes with provenance only from the current `main` commit when a matching `v<version>` tag exists. It pins an npm CLI that meets the trusted-publishing floor, rejects already-published versions, and verifies registry version plus integrity afterward. Trusted publisher authentication is the default and receives no long-lived token. A token fallback remains an explicit workflow choice until npm trusted publishing is configured and verified; selecting it must be treated as a security exception, not a silent fallback. Workflow readiness is not proof that an unpublished release already has provenance.
