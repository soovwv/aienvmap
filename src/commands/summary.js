...
export function renderSummary(status = {}, manifest = {}) {
  ...
  return [
    ...
    `- quality signals: ${qualitySignals.status || "release-candidate"} / ${qualityPrinciples.join(", ") || "AI-friendly, simple, lightweight"}`,
    `- release readiness: ${releaseReadiness.target || "0.2.0"} / ${releaseReadiness.status || "release-candidate"} / ${publishGate.status || "ready-for-final-check"} / ${currentBatch.status || "reviewed"} / ${releaseChecks[0] || "npm run release:check passes locally"}`,
    "",
    ...
    "## Quality signals",
    "",
    `- status: ${qualitySignals.status || "release-candidate"}`,
