...
- Current Python manager proof: `--full-packages` uses uv's offline managed-Python JSON list to prove uv ownership for exact version/root matches while keeping removal authorization false.
- Next package-manager coverage: add pyenv/mise manager proof, pnpm, Yarn/Corepack, standalone pip/uv/pipx entry-point conflicts, and Conda without adding runtime dependencies.
...
- Current Java build-tool routing: prefer project Maven/Gradle wrappers, retain only bounded version/JVM identity evidence, link exact homes strongly or unique major versions conservatively, and flag ambiguous or active-Java divergence for AI review.
