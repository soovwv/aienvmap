...
- Current APM integration: `apm.yml` and one compact skill distribute the advisory workflow to the shared `.agents/skills` and Claude skill roots; the recommended skill-subpath install keeps the package footprint bounded, with no hooks, MCP server, executable deployment, or automatic aienvmap/npm install.
