...
- Current APM integration: `apm.yml` and one compact skill distribute the advisory workflow to the shared `.agents/skills` and Claude skill roots; a clean consumer CI gate verifies bounded skill-subpath placement and native-pointer coexistence, with no hooks, MCP server, executable deployment, or automatic aienvmap/npm install.
